  ...
  try {
    ...
    if (touristSpotSnapshot.exists()) {
      ...
      if (touristNameElement) {
        ...
        const ratingsRef = doc(db, 'ratings', 'Hotel', 'Hotel_reviews', documentID);
...
async function getReviews(documentID) {
  try {
    const allReviews = [];
    const reviewsRef = collection(db, 'ratings', 'Hotel', 'Hotel_reviews', documentID, 'user_reviews');
    ...
    for (const userDoc of userDocs.docs) {
      ...
      const userReviewRef = doc(db, 'ratings', 'Hotel', 'Hotel_reviews', documentID, 'user_reviews', userID);
