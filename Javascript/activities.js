...
formCreate.addEventListener('submit', async (e) => {
  e.preventDefault();
  if (validateInputs([ name, description, location, photos])) {
    try {
      const photoFile = photos.files[0];
      const photoRef = ref(storage, `activities/${photoFile.name}`);
...
formEdit.addEventListener('submit', async (e) => {
  e.preventDefault();
  if (validateInputs([name1, description1, location1, photos1])) {
    try {
